...
GameManager::GameManager(SDL_Renderer* &renderer) 
{
    ...
    egBoard = loadImage(renderer, "Assets/endgame.png");

    hsBoard = loadImage(renderer, "Assets/newHighscore.png");
...
void GameManager::runEGBoard(SDL_Renderer* &renderer) 
{
    ...
    SDL_RenderCopy(renderer, egBoard, NULL, &dsRect);
    yesBut->renderButton(renderer);
    noBut ->renderButton(renderer);
    if (newRecord) 
    {
        SDL_RenderCopy(renderer, hsBoard, NULL, &dsRect);
