            ...
            for (int i = 0; i < MAP_HEIGHT; i++) {
                for (int j = 0; j < MAP_WIDTH; j++) {
                    ...
                    if (premanMap[i][j] > 31 || premanMap[i][j] < 0) {
                        std::cout << "Map : Wrong Tile!" << std::endl;
                        return;
                    }
                }
            }

            std::cout << "Map : Map read successfully!" << std::endl;
        }
        else std::cout << "Map : Error reading file!" << std::endl;
    }
    ...
    for (int x = 0; x < MAP_WIDTH; x++) 
    {
        for (int y = 0; y < MAP_HEIGHT; y++) 
        {
            ...
            for (int startDir = 0; startDir < 4; startDir++ ) 
            {
                ...
                for (int i = 0; i < MAP_WIDTH * MAP_HEIGHT; i++) dist[xn * MAP_HEIGHT + yn][i][startDir] = dis[i];
            }
        }
    }
