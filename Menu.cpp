...
Menu::Menu(const int baseScrPosX, const int baseScrPosY, const int totalButton, const int buttonWidth, const int buttonHeight) 
{
    ...
    for (int i = 0; i < 3; i++) 
    {
        howToPlayTexture[i] = NULL;
    }
    highScoreTexture = NULL;
    scoreText = NULL;
    arrowHSPos = 0;
...
void Menu::init(SDL_Renderer* &renderer, const std::string imgPath, std::vector<std::string>& buttonText) 
{
    SDL_Surface* Image = IMG_Load(imgPath.c_str());
    if (Image == NULL)
        std::cout << "Menu : " <<  IMG_GetError() << std::endl;
    else {
        menuTexture = SDL_CreateTextureFromSurface(renderer, Image);
        SDL_FreeSurface(Image);
        howToPlayTexture[0] = loadImage(renderer, "Assets/How to Play1.png");
        howToPlayTexture[1] = loadImage(renderer, "Assets/How to Play2.png");
        howToPlayTexture[2] = loadImage(renderer, "Assets/How to Play3.png");
        Image = IMG_Load("Assets/High Scores.png");
        highScoreTexture = loadImage(renderer, "Assets/High Scores.png");
...
void Menu::render(SDL_Renderer* &renderer, const std::vector<std::string> &scoreData) 
{
    ...
    {
        SDL_RenderCopy(renderer, howToPlayTexture[currentHTPid], NULL, NULL);
    }
    else if (currentMenuStatus == HIGH_SCORES) 
    {
        // Render Menu HighScore
    ...
    else 
    {
        ...
        for (int i = 0; i < TOTAL_BUTTON; i++) menuButton[i]->renderButton(renderer);
    }
}
