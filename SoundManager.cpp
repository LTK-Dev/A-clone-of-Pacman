...
{
    for (int i = 0; i < 11; i++) soundEffect[i] = NULL;
    eatDotTime = 0;
    oldMoveType = -1;
    newMoveType = MOVE_0;
    ghostTurnBlue = false;
...
{
    soundEffect[ MOVE_0 ] = Mix_LoadWAV("Assets/move 0.wav");
    soundEffect[ MOVE_1 ] = Mix_LoadWAV("Assets/move 1.wav");
    soundEffect[ MOVE_2 ] = Mix_LoadWAV("Assets/move 2.wav");
    soundEffect[ MOVE_3 ] = Mix_LoadWAV("Assets/move 3.wav");
    soundEffect[ START  ] = Mix_LoadWAV("Assets/start.wav");
    soundEffect[ DEAD   ] = Mix_LoadWAV("Assets/dead2.wav");
    soundEffect[ WINNING ] = Mix_LoadWAV("Assets/next level.wav");
    soundEffect[ EAT_DOT ] = Mix_LoadWAV("Assets/eat dot.wav");
    soundEffect[ EAT_GHOST ] = Mix_LoadWAV("Assets/eat ghost.wav");
    soundEffect[ GHOST_GO_HOME ] = Mix_LoadWAV("Assets/ghost go home.wav");
    soundEffect[ GHOST_TURN_BLUE ] = Mix_LoadWAV("Assets/ghost turn blue.wav");

    for (int i = 0; i < 11; i++)
    {
        if (soundEffect[i] == NULL)
        {
            std::cout << "Sound : " << Mix_GetError() << std::endl;
        }
    }
